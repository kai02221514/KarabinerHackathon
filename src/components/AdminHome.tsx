...
export default function AdminHome({ applications, user, onNavigate, onLogout, onEditForm }: AdminHomeProps) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          {applications.filter(app => app.isPublished).slice(0, 3).length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              公開中のフォームはありません
            </div>
          ) : (
            <div>
              {applications.filter(app => app.isPublished).slice(0, 3).map((app, index) => (
                <div 
                  key={app.id} 
                  className={`p-4 hover:bg-gray-50 cursor-pointer ${
                    index !== applications.filter(a => a.isPublished).slice(0, 5).length - 1 ? 'border-b border-gray-200' : ''
                  }`}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-gray-600 text-sm">{app.submissionMethod}</span>
                      </div>
                    </div>
                    <Button 
                      variant="ghost" 
